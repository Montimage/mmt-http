pipeline {
    agent {
        docker {
            image 'ubuntu'
            args '-u root:sudo -v $HOME/workspace/mmt-http:/mmt-http -v $HOME/workspace/mmt-sdk:/mmt-sdk'
        }
    }
    stages {
        ...
        stage("compile") {
            steps {
                bitbucketStatusNotify(buildState: 'INPROGRESS')
                sh 'cd /mmt-http/ && gcc -g -o mmtHTTP mmtHTTP.c -I /opt/mmt/dpi/include -L /opt/mmt/dpi/lib -lmmt_core -ldl -lpcap'
                // sh 'cd /mmt-http/ && make'
            }
        }
